...
stat$NonWhite = stat$Black + stat$Hispanic +stat$Asian + stat$Pacific

Races = subset(stat, select = c("geo_id","White","NonWhite"))
...
names(Shootings)[names(Shootings) == 'X2'] <- 'White'
...
write.csv(Shootings, file = "csvs/Binary_Tract_RacesOfVictims.csv",row.names=FALSE)
write.csv(Races, file = "csvs/Binary_Tract_RacesOfCounties.csv",row.names=FALSE)
