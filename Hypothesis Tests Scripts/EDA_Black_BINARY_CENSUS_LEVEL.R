...
stat$NonBlack = stat$White + stat$Hispanic +stat$Asian + stat$Pacific

Races = subset(stat, select = c("geo_id","NonBlack","Black"))
...
names(Shootings)[names(Shootings) == 'X2'] <- 'NonBlack'
names(Shootings)[names(Shootings) == 'X3'] <- 'Black'
...
for(i in 1:nrow(stat)) {
  if(stat$raceethnicity[i] == "Hispanic"){
    ...
    Shootings$NonBlack[index] <- Shootings$NonBlack[index] + 1
  }
...
write.csv(Shootings, file = "csvs/Black_Binary_Tract_RacesOfVictims.csv",row.names=FALSE)
write.csv(Races, file = "csvs/Black_Binary_Tract_RacesOfCounties.csv",row.names=FALSE)
