...
drop = c("x.1","geo_id","x","streetaddress","latitude", "longitude","state_fp","county_fp","tract_ce","county_id","lawenforcementagency",
         "incomeerr", "incomepercaperr","income", "incomepercap", "year", "namelsad", "state.1", 
         "month", "day", "totalpop","men", "women","employed")
merged1 = merged1[,!(names(merged1) %in% drop)] 
...
M1 = glmer(raceethnicity ~ age +  (1|county), 
           family = binomial("logit"), data = merged1)
summary(M1)

M2 = glmer(raceethnicity ~ age + gender  + (age + gender|state), 
           family = binomial("logit"), data = merged1)
summary(M2)

anova(M1,M2)

M3 = glmer(raceethnicity ~ age + gender + cause + (1|county), 
           family = binomial("logit"), data = merged1)
summary(M3)

anova(M2,M3)

M4 = glmer(raceethnicity ~ age + gender + armed + (1|county), 
           family = binomial("logit"), data = merged1)
summary(M4)
